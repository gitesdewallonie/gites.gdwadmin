...
class ModifCommune(BrowserView):
    ...
    def table_com(self):
        ...
        query = session.query(Commune).join('province').join('regionGuide').join('maisonTourisme').filter(Commune.com_pk == self.com_pk)
        return query.first()
    ...
    def commune(self):
        ...
        query = session.query(Commune.com_nom,
                              ...
                              Commune.com_cp,
                              Commune.com_ins,
                              Commune.com_id).filter(Commune.com_pk == self.com_pk)
        return query.first()
